...
const rootDbPath = path.join(__dirname, 'banco-terreiro.sqlite'); 
const persistentDbPath = path.join(dataDir, 'banco-terreiro.sqlite'); 

// Garante que as pastas existam
if (!fs.existsSync(dataDir)) { fs.mkdirSync(dataDir, { recursive: true }); }
if (!fs.existsSync(uploadDir)) { fs.mkdirSync(uploadDir, { recursive: true }); }

// MIGRAÇÃO: Copia o banco do root para a pasta de dados se for o primeiro acesso
...
// Banco de dados conectado na pasta persistente
...
app.get('/api/conteudos/:chave', (req, res) => { db.get(`SELECT * FROM conteudos WHERE chave = ?`, [req.params.chave], (err, row) => res.json({ sucesso: true, conteudo: row })); });
...
app.get('/api/mediuns', (req, res) => { db.all(`SELECT * FROM usuarios`, [], (err, rows) => res.json({ sucesso: true, lista: rows || [] })); });
...
app.post('/api/atualizar-status', (req, res) => { db.run(`UPDATE usuarios SET status_mensalidade = ? WHERE id = ?`, [req.body.novoStatus, req.body.id], () => res.json({ sucesso: true })); });
...
app.get('/api/avisos', (req, res) => { db.all(`SELECT * FROM avisos ORDER BY id DESC`, [], (err, rows) => res.json({ sucesso: true, avisos: rows || [] })); });
app.post('/api/avisos', (req, res) => { db.run(`INSERT INTO avisos (titulo, mensagem, tipo) VALUES (?, ?, ?)`, [req.body.titulo, req.body.mensagem, req.body.tipo], () => res.json({ sucesso: true })); });
app.delete('/api/avisos/:id', (req, res) => { db.run(`DELETE FROM avisos WHERE id = ?`, [req.params.id], () => res.json({ sucesso: true })); });

app.get('/api/agenda', (req, res) => { db.all(`SELECT * FROM agenda ORDER BY id ASC`, [], (err, rows) => res.json({ sucesso: true, agenda: rows || [] })); });
